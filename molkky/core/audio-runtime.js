(function(){'use strict';if(!window.AudioManager)throw Error('AudioManager missing');var recent=[],bgmRequested=false;
function stageBgmId(){var s=window.StageManager&&StageManager.get();return s&&s.bgm}
function startBgm(){var id=stageBgmId();if(!id)throw Error('stage BGM not configured');bgmRequested=true;return AudioManager.startBgm(id)}
function ensureBgm(){var id=stageBgmId();if(!id||!bgmRequested)return AudioManager.resume();return AudioManager.resume().then(function(){return AudioManager.startBgm(id)})}
