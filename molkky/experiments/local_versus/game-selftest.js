...
window.addEventListener('load',function(){setTimeout(function(){try{
  ...
  CharacterManager.renderAll('normal');decodeImages(character.images,function(error){if(error)return fail(error.message);try{StageRuntime.draw(ctx,W,H);draw();pass()}catch(drawError){fail(drawError&&drawError.message?drawError.message:String(drawError))}})
